...
def deletar_flashcard(request, id):
    ...
    if flashcard.user_id != request.user.id:
        messages.add_message(
            request, constants.ERROR, 'O flashcard que solicitou exclusão não é seu!'
        )
        return redirect('/flashcard/novo_flashcard')     
    else:
        flashcard.delete()
...
def iniciar_desafio(request):
    if request.method == 'GET':
        ...
        if flashcards.count() < int(qtd_perguntas):
            messages.add_message(
                request, constants.ERROR, 'A quantidade de questões que está solicitando não existe no banco de dados!'
            )
